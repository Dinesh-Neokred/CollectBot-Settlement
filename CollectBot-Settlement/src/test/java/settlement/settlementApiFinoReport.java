...
public class settlementApiFinoReport extends login {
    ...
    public void createSettlement() throws EncryptedDocumentException, IOException {
        ...
        ReadingSettlementDataFromFinoReport settlementDate = new ReadingSettlementDataFromFinoReport();
        ...
        for (int i = 0; i < LastRowNumber; i++) {
            ...
            filePath = "C:\\Users\\Dinesh M\\Downloads\\Neokred - Cycle Wise UTR 30-04-2024.xlsx";
            ...
            double chargebackHold=settlementDate.chargeBackHold;
            double chargebackRelease=settlementDate.chargeBackRelease;
            ...
            if (settledAmount >= 1) {
                requestPayload = given()
                        ...
                        .header("chargebackRelease",chargebackRelease)
                        .header("chargebackHold",chargebackHold)
                // .log().all()
                ;
                ...
                data.getRow(3 + i).getCell(19).setCellValue(beforeDebitBalance);
                ...
                logger.info(
                        "Settlement Record Create API Response====>" + utr +" "+ createSettlementApi.then().log().all());

                logger.info("Calling get Before DebitBalance Method..." + utr+" ");
                double aftereDebitBalance = Balance.getAfterDebitBalance(clientId, auth);
                logger.info("Storing After DebitBalance In Excel..." + utr+" ");
                data.getRow(3 + i).getCell(20).setCellValue(aftereDebitBalance);
                ...
                data.getRow(3 + i).getCell(21).setCellValue(response);

                if (settlementMessage.equalsIgnoreCase("Settlement record created successfully")) {
                    logger.info("Settlement record created successfully, proceeding with Revenue API call." + utr+" ");
                    ...
                    logger.info("Revenue Record Create API Response====>" + utr +" "+ createRevenueApi.then().log().all());
                    ...
                    data.getRow(1 + i).getCell(22).setCellValue(createRevenueResponse);
                }
